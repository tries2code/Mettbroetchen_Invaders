...
public:
	...
	static void kill_hostiles(void* addr) {					//this is where hostiles and laser-projectiles get deleted/hidden in case of a hit
		...
		MB_Invaders* mbiw = (MB_Invaders*)addr;
		Fl::add_timeout(1, kill_hostiles, addr);
	...
	static void speed_up(void* addr) {
	...
	int handle(int event) {							//fltk handle for keyboard input
		...
		case FL_KEYUP:
			...
			if (Fl::event_key() == 97 || Fl::event_key() == 65361)if(ship.get_pos().first>x-25)ship.move(-10, 0);				//move to the left
			if (Fl::event_key() == 100 || Fl::event_key() == 65363)if (ship.get_pos().first<length/12*11-25)ship.move(10, 0);	//move to the right
